...
export default function Chat() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: "assistant",
      content:
        "Hello! I'm connected to your filesystem. Configure your Local LLM in settings to start chatting.",
    },
  ]);
  ...
  const callLlm = async (messages: Message[], currentConfig: LlmConfig) => {
    ...
    if (currentConfig.provider === "anthropic")
      endpoint = `${currentConfig.baseUrl}/messages`;
      ...
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Interact with your filesystem using{" "}
        ...
        <div className="flex items-center gap-2">
          {!config && (
            <Link
              to="/settings"
              className="text-yellow-500 flex items-center gap-1 text-sm bg-yellow-500/10 px-3 py-1 rounded-full"
            >
          ...
          <div
            className={`flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium ${isConnected ? "bg-green-500/10 text-green-500" : "bg-red-500/10 text-red-500"}`}
          >
            <div
                  ...
                  <div className="mt-2 p-2 bg-black/10 rounded text-xs font-mono overflow-x-auto">
                    ...
                    {msg.toolCall.result && (
                      <div className="text-green-300 border-t border-white/10 pt-1 mt-1">
                        Result:{" "}
                        {JSON.stringify(msg.toolCall.result).substring(0, 300)}
                        ...
                      </div>
                    )}
                  </div>
        ...
        <div className="p-4 border-t border-border bg-card/50">
          <form onSubmit={handleSubmit} className="flex gap-2">
            <input
              ...
              placeholder={
                config
                  ? "Ask to list files, read code, or check status..."
                  : "Enter your message (Defaults to Ollama/Llama3.2:1b)..."
              }
